...
public:
    ...
    Expression<T> derivative(const std::string &varName) const override {
        ...
        switch (op) {
            case Expression<T>::BinaryOp::Add:
                return dl + dr;
            case Expression<T>::BinaryOp::Subtract:
                return dl - dr;
            case Expression<T>::BinaryOp::Multiply:
                // (f * g)' = f' * g + f * g'
                return (dl * Expression<T>(right->clone())) + (Expression<T>(left->clone()) * dr);
            case Expression<T>::BinaryOp::Divide:
                // (f / g)' = (f' * g - f * g') / (g^2)
                return (dl * Expression<T>(right->clone()) - Expression<T>(left->clone()) * dr)
                       / (Expression<T>(right->clone()) ^ Expression<T>(2.0));
            case Expression<T>::BinaryOp::Power:
                // Правило для f(x)^n (n - константа)
